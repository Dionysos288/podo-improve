...
import {
	EnhancedSTLViewer,
	type EnhancedSTLViewerRef,
} from '@/src/features/design/components/EnhancedSTLViewer';
import { STLSelector } from '@/src/features/design/components/STLSelector';
...
import { MiniSTLPreview } from '@/src/features/design/components/MiniSTLPreview';
import {
	buildInsolePlan,
	type LandmarkPoints,
...
export default function DesignPage() {
	...
	if (isLoading) {
		return (
			<div className="flex h-full items-center justify-center">
				<p className="text-ui-muted">Laden...</p>
			</div>
		);
	}
	...
	if (error) {
		return (
			<div className="flex h-full items-center justify-center">
				<p className="text-red-500">Fout bij laden: {error.message}</p>
			</div>
		);
	}
	...
	const handleCancelPointPick = () => {
		setPointSelections({});
		setPointStepIndex(0);
		setWorkflowStep('base');
	};

	const handlePointPicked = (point: [number, number, number]) => {
	...
	const handleExportSTL = () => {
		...
		if (!geometry) {
			console.warn('No insole geometry available to export');
			alert(
